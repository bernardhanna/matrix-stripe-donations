...
test('donate page proceeds to checkout step', async ({ page }) => {
  ...
  await page.locator('#first_name').fill('Flow');
  ...
  await page.route('**/wp-json/matrix-donations/v1/checkout-intent', async route => {
    await route.fulfill({
      ...
      body: JSON.stringify({ success: true, checkoutUrl: '/thank-you/?session_id=mock_flow' })
    });
  });

  await page.locator('#salesforceForm button[type="submit"]').click();
  await intentRequestPromise;
  await expect(page).toHaveURL(/\/thank-you\/\?session_id=mock_flow$/);
});
